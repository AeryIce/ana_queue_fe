  ...
  return (
    <div className="container-page">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {loading
            ...
            : (data.active ?? []).map((t: Ticket) => {
                // Pastikan string untuk dipakai action & title
                const code = (t.code ?? t.id) ?? "";
                const hasCode = code.length > 0;

                return (
                  <div
                    key={(t.id ?? t.code) ?? `act-${Math.random().toString(36).slice(2)}`}
                    className="rounded-2xl p-3 bg-white/5 border border-white/10"
                  >
                    ...
                    {t.name && (
                      <div className="text-sm opacity-80 mt-1 line-clamp-1">{t.name}</div>
                    )}
                  </div>
                );
              })}
      ...
      <section className="mt-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
          {loading
            ...
            : data.next.map((t: Ticket, idx: number) => (
                <div
                  key={(t.code ?? t.id) ?? `next-${idx}`}
                  className="rounded-xl p-3 bg-white/5 border border-white/10"
                >
                  <QueueCard t={t} />
                </div>
      ...
      <footer className="mt-8 text-xs opacity-70">
        Dipersembahkan oleh Periplus • v-02102025
      </footer>
    </div>
  );
}
