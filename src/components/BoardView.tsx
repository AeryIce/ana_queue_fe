...
import { normalizeBoard } from '@/lib/queueApi';

export default function BoardView({ board }: { board: BoardResponse | null }) {
  if (!board) return null;

  // Normalisasi biar aman dari "possibly undefined"
    ...
    <div className="space-y-4">
      <section>
        ...
        <ul className="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {safe.active.map((t: Ticket) => (
            <li
              key={t.id ?? t.code ?? Math.random().toString(36)}
              className="rounded-xl p-3 bg-white/5 border border-white/10"
            >
              <div className="text-lg font-bold tabular-nums">{t.code ?? t.name ?? (t.id ? t.id.slice(0, 6) : '-')}</div>
      ...
      <section>
        ...
        <ul className="mt-2 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
          {safe.next.map((t: Ticket) => (
            <li
              key={t.id ?? t.code ?? Math.random().toString(36)}
              className="rounded-xl p-3 bg-white/5 border border-white/10"
            >
              <div className="text-base font-bold tabular-nums">{t.code ?? t.name ?? (t.id ? t.id.slice(0, 6) : '-')}</div>
            </li>
          ))}
        </ul>
      </section>
