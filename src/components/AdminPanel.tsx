      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            ...
            if (!code) return setMsg('Masukkan code tiket.');
            run(() => queueApi.call(code));
          }}
        >
          Call by Code
        </button>
          ...
          <div className="space-y-3">
            {data?.active.map(t => (
              <div key={t.id} className="p-3 rounded-xl border flex items-center justify-between gap-3">
                <div>
                  <div className="font-semibold">{t.code} <span className="text-xs px-2 py-0.5 rounded bg-green-100">{t.status}</span></div>
                  <div className="text-xs text-gray-500">{t.name}</div>
                </div>
                <div className="flex items-center gap-2">
                  {t.status === 'CALLED' && (
                    <button className="px-3 py-1 rounded bg-amber-500 text-white disabled:opacity-50"
                      disabled={busy}
                      onClick={() => run(() => queueApi.inProcess(t.code))}>
                      In-Process
                    </button>
                  )}
                  {(t.status === 'CALLED' || t.status === 'IN_PROCESS') && (
                    <button className="px-3 py-1 rounded bg-emerald-600 text-white disabled:opacity-50"
                      disabled={busy}
                      onClick={() => run(() => queueApi.done(t.code))}>
                      Done
                    </button>
                  )}
                  {t.status === 'CALLED' && (
                    <button className="px-3 py-1 rounded bg-gray-700 text-white disabled:opacity-50"
                      disabled={busy}
                      onClick={() => run(() => queueApi.skip(t.code))}>
                      Skip
                    </button>
                  )}
                </div>
            ...
            {(!data || data.active.length === 0) && <div className="text-sm text-gray-500">Belum ada yang dipanggil.</div>}
          </div>
        </section>
          ...
          <div className="space-y-2">
            {data?.next.map(t => (
              <div key={t.id} className="p-3 rounded-xl border flex items-center justify-between">
                ...
                <button className="px-3 py-1 rounded bg-blue-600 text-white disabled:opacity-50"
                  disabled={busy}
                  onClick={() => run(() => queueApi.call(t.code))}>
                  Call
                </button>
              </div>
            ))}
            {(!data || data.next.length === 0) && <div className="text-sm text-gray-500">Tidak ada antrean.</div>}
          </div>
        </section>
      </div>
